...
while (beta<betaf)
    ...
    while((abs(a0-a)+abs(b0-b)+abs(c0-c)+sqrt((t10-t1)^2+(t20-t2)^2)+abs(theta0-theta))<ep1 && iters1<I0)
        ...
        while (itersNewton < maxItersNewton)
            ...
            % Find the value of the first derivative of the objective function at current params
            tmpX1 = repmat(X(1,:)',1,Nk) - t1;
            tmpX2 = repmat(X(2,:)',1,Nk) - t2;
            A = getA(a,b,c,theta);
            tmpY = A*Y;
            ...
            % Find the value of the second derivative of the objective function at
            % current params
        ...
        sc = [exp(a) 0; 0 exp(a)];  % Scaling matrix
        rot = [cos(theta) -sin(theta); sin(theta) cos(theta)];  % Rotation matrix
        sh2 = [cosh(c) sinh(c); sinh(c) cosh(c)];   % Shear matrix 2
        ...
        while (itersNewton < maxItersNewton)
            ...
            A_der1_b = sc*rot*sh1Der1*sh2;    % First derivative of A wrt a
            tmpYDer1 = A_der1_b*Y;
            tmpYDer11 = repmat(tmpYDer1(1,:),Nj,1);
            tmpYDer12 = repmat(tmpYDer1(2,:),Nj,1);
            ...
            A_der2_b = sc*rot*sh1Der2*sh2;    % Second derivative of A wrt a
            tmpYDer2 = A_der2_b*Y;
            tmpYDer21 = repmat(tmpYDer2(1,:),Nj,1);
            tmpYDer22 = repmat(tmpYDer2(2,:),Nj,1);
            denom = 2*sum(sum(mHat(1:Nj,1:Nk).*((tmpY1-tmpX1).*tmpYDer21 + (tmpYDer11-tmpX1).*tmpYDer11 + (tmpY2-tmpX2).*tmpYDer22 + (tmpYDer21-tmpX2).*tmpYDer21))) + 2*gamma;
        ...
        while (itersNewton < maxItersNewton)
            ...
            A_der1_c = sc*rot*sh1*sh2Der1;    % First derivative of A wrt a
            tmpYDer1 = A_der1_c*Y;
            tmpYDer11 = repmat(tmpYDer1(1,:),Nj,1);
            tmpYDer12 = repmat(tmpYDer1(2,:),Nj,1);
            ...
            A_der2_c = sc*rot*sh1*sh2Der2;    % Second derivative of A wrt a
            tmpYDer2 = A_der2_c*Y;
            tmpYDer21 = repmat(tmpYDer2(1,:),Nj,1);
            tmpYDer22 = repmat(tmpYDer2(2,:),Nj,1);
            denom = 2*sum(sum(mHat(1:Nj,1:Nk).*((tmpY1-tmpX1).*tmpYDer21 + (tmpYDer11-tmpX1).*tmpYDer11 + (tmpY2-tmpX2).*tmpYDer22 + (tmpYDer21-tmpX2).*tmpYDer21))) + 2*gamma;
